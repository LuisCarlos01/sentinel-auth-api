...
1. **Emissão** (`login`/`refresh`): o service de autenticação, após validar credenciais (ver [`spring-boot-argon2id.md`](spring-boot-argon2id.md)), monta claims (`sub`, `roles`, `iat`, `exp`) e chama `Jwts.builder()...signWith(key, Jwts.SIG.HS256).compact()` para gerar o access token. Ver [`jjwt.md`](../technologies/jjwt.md) para o ponto em aberto sobre se o refresh token também é um JWT ou um token opaco.
